...
const SideBar:React.FC<Props> = ({ categories })=>{
    ...
    return (
        <div id="room-list-container" className={`fixed top-0 bottom-0 left-0 ${visibility?"w-48":"w-0"} duration-75 overflow-hidden z-10 bg-black text-white border border-blue-400 md:static md:w-auto md:basis-72 md:grow-0 md:shrink-0`}>
                    <div id="b-btn-container" className={`${visibility?"absolute top-0 right-0":"fixed top-0 left-[10px]"}  text-white ${visibility?`${style.change}`:`${style.container}`} duration-1000 md:hidden`} onClick={()=>setVisibility(!visibility)}>
                        <div id="div1" className={`h-[3px] w-[35px] my-[6px] mr-[2px] ml-[0px] ${visibility?"bg-white":"bg-black"}`}></div>
                        <div id="div2" className={`h-[3px] w-[35px] my-[6px] mr-[2px] ml-[0px] ${visibility?"bg-white":"bg-black"}`}></div>
